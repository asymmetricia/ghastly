## ghastly config start-options-flow

start a config flow with with the given handler ID as the handler
...
```
ghastly config start-options-flow [handler-id] [flags]
```

### Options

```
  -h, --help   help for start-options-flow
```
